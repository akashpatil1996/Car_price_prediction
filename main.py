...
name = ['Hyundai Santro Xing', 'Mahindra Jeep CL550', 'Hyundai Grand i10',
...
year = ['2007', '2006', '2014', '2012', '2013', '2016', '2015', '2010', '2017', '2008', '2018', '2011',
        '2019', '2009', '2005', '2000', '2003', '2004', '1995', '2002', '2001', '2020', '2021', '2022', '2023']

pipe = pickle.load(open('pipe.pkl','rb'))

st.title('Car Price Predictor')

st.markdown("<h5 style='text-align: left; color: grey;'>Enter the details below to predict your car price</h5>", unsafe_allow_html=True)


Brand = st.selectbox('Brand',sorted(company))

Model = st.selectbox('Model',sorted(name))

Year = st.selectbox('Year of Purchase',sorted(year,reverse=True))

Kms_driven = st.number_input('Kms driven')

Fuel_type = st.selectbox('Fuel Type',sorted(fuel_type))

if st.button('Predict Price'):
    input_df = pd.DataFrame({'name':[Model],'company':[Brand],'year':[Year],'kms_driven':[Kms_driven],'fuel_type':[Fuel_type]})

    result = pipe.predict(input_df)
    st.success('₹ ' + str(round(result[0])) + ' is the predicted price of the car')
